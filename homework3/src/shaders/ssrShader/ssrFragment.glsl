...
bool RayMarch(vec3 ori, vec3 dir, out vec3 hitPos) {
  ...
  for (int i=0; i<MAX_STEPS; i++) {
    vec3 currentPos = ori + dir * step;
    vec2 currentUV = GetScreenCoordinate(currentPos);
    if (length(currentUV) > 1.0) { // out of the screen
      break;
    }
    float currentDepth = GetDepth(currentPos);
    float bufferDepth = GetGBufferDepth(currentUV);
    if (currentDepth > bufferDepth - EPS) {
      hitPos = currentPos;
      result = true;
      break;
    }
...
void main() {
  ...
  for (int i=0; i<SAMPLE_NUM; i++) {
    float pdf;
    Rand1(s);
    vec3 dir = normalize(TBN * SampleHemisphereUniform(s, pdf));

    vec3 hitPos;
    if (RayMarch(vPosWorld.xyz, dir, hitPos)) {
      vec3 samplebsdf = EvalDiffuse(dir, wo, uv);

      vec2 hituv = GetScreenCoordinate(hitPos);
      vec3 hitwi = normalize(uLightDir - (hitPos - vPosWorld.xyz));
      vec3 hitbsdf = EvalDiffuse(hitwi, dir, hituv);

      L_indirect += samplebsdf * hitbsdf * EvalDirectionalLight(hituv) / pdf;
    }
  }
