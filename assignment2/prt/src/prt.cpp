            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            double basicFunction = sh::EvalSH(l, m, dir.cast<double>().normalized());
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * basicFunction * dw;
                        }
                    }
...
public:
    ...
    PRTIntegrator(const PropertyList& props)
    {
        ...
        type = props.getString("type", "unshadowed");
        if (type == "unshadowed")
        {
    ...
    virtual void preprocess(const Scene* scene) override {
        ...
        auto transPath = cubePath / ("transport_" + type + ".txt");
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++) {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed) {
                    ...
                    return H > 0 && !scene->rayIntersect(ray) ? H : 0;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            MatrixXf indirectTransportCoeffs = MatrixXf::Zero(SHCoeffLength, mesh->getVertexCount());
            for (int i = 0; i < mesh->getVertexCount(); i++) {
                const Point3f& v = mesh->getVertexPositions().col(i);
                const Normal3f& n = mesh->getVertexNormals().col(i);
                std::unique_ptr<std::vector<double>> vertexCoeffs = calculateIndirectTransportCoeffs(&m_TransportSHCoeffs, scene, v, n, 1);
                for (int j = 0; j < SHCoeffLength; j++) {
                    indirectTransportCoeffs.col(i).coeffRef(j) += (*vertexCoeffs)[j];
                }
            }
            m_TransportSHCoeffs += indirectTransportCoeffs;
        }
    ...
    std::unique_ptr<std::vector<double>> calculateIndirectTransportCoeffs(const MatrixXf* TransportCoeffs, const Scene* scene, const Point3f& v, const Normal3f& n, int depth) {

        std::unique_ptr<std::vector<double>> indirectTransportCoeffs(new std::vector<double>());
        indirectTransportCoeffs->assign(SHCoeffLength, 0.0);

        if (depth > m_Bounce) {
            return indirectTransportCoeffs;
        }
        ...
        for (int t = 0; t < sample_side; t++) {
            for (int p = 0; p < sample_side; p++) {
                ...
                if (H > 0 && scene->rayIntersect(ray, intersection)) {
                    ...
                    Normal3f interpNormal = Normal3f(normals.col(triangleIndexs.x()).normalized() * bary.x() +
                        normals.col(triangleIndexs.y()).normalized() * bary.y() +
                        normals.col(triangleIndexs.z()).normalized() * bary.z()).normalized();
                    auto nextBounce = calculateIndirectTransportCoeffs(TransportCoeffs, scene, intersectPoint, interpNormal, depth + 1);

                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            int SHIndex = sh::GetIndex(l, m);
                            auto indirect = (TransportCoeffs->col(triangleIndexs[0]).coeffRef(SHIndex) * bary.x() +
                                TransportCoeffs->col(triangleIndexs[1]).coeffRef(SHIndex) * bary.y() +
                                TransportCoeffs->col(triangleIndexs[2]).coeffRef(SHIndex) * bary.z() +
                                (*nextBounce)[SHIndex]) * H;
                            (*indirectTransportCoeffs)[SHIndex] += indirect;
                        }
                    }
                }
            }
        ...
        for (unsigned int i = 0; i < indirectTransportCoeffs->size(); i++) {
            (*indirectTransportCoeffs)[i] *= weight;
        }
