...
async function GAMES202Main() {
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		precomputeLT[i] = [];
		for (let j = 0; j < shadowtype.length; j++) {
			await this.loadShaderFile(envmap[i] + "/transport_"+shadowtype[j]+".txt").then(result => {
				val = result;
			});
			let preArray = val.split(/[(\r\n)\r\n' ']+/)
			precomputeLT[i][j] = [];
			for (let k = 1; k <= Number(preArray.length) - 2; k++) {
				precomputeLT[i][j][k - 1] = Number(preArray[k]);
			}
		}
		...
		precomputeL[i].pop();
		for (let j = 0; j < 9; j++) {
			let lineArray = precomputeL[i][j].split(' ');
			for (let k = 0; k < 3; k++) {
	...
	function createGUI() {
		...
		panelModel.add(guiParams, 'shadowtypeId', { 'Unshadowed':0, 'Shadowed': 1, 'Interreflection': 2 }).name('Shadowtype Name');
		panelModel.open();
	}
